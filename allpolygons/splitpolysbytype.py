...
def shallow_is_inside(coords, file_name, area, n):
    ...
    n = int(n)
    if n <= 21:
        infile = open("ssconvertedlong.txt")
    
    if n > 21:
        infile = open("thrustconvertedlong.txt")
...
def condensingrepeats(area, total):
    ...
    newfileKostrov = open("%sKostrovsbytype.txt" % area, "w")
    newfilepolygons = open("%spolygonsbytype.txt" % area, "w")
    counter = 19
    linecounter = 0
    linenumbers = []
    while counter <= total:
...
def uncondensingrepeats(area, total, linenumbers):
    ...
    print(linenumbers)
    Kosinfile = open("m0%sKostrovsbytype.txt" % area)
    Kostrovdata = []
    polygonmwinfile = open("mw%spolygonsbytype.txt" % area)
    polygonm0infile = open("m0%spolygonsbytype.txt" % area)
    polygonmwdata = []
    polygonm0data = []
    ...
    while counter <= total:
        newfileKostrov = open("m0%sKostrov%i.txt" % (area, actualcounter), "w")
        newfilemwpolygon = open("mw%spolygon%i.txt" % (area, actualcounter), "w")
        newfilem0polygon = open("m0%spolygon%i.txt" % (area, actualcounter), "w")
